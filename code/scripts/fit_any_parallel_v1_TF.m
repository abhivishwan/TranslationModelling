function [Xfit, nLL, BIC, AIC] = fit_any_parallel_v1_TF( sub, data_flag, model_flag, n_fit )
...
switch model_flag
    ...
    case 3 % Pure Landmark Navigation
        model = @(x, tar, head_t, feed, head_f) ...
            lik_pureLandmark_v1_TF(x, tar, head_t, feed, head_f);
        % n_parameters = 4;
        % v = 3;
        % X0(v) = 1; LB(v) = 1; UB(v) = 1;
        n_parameters = 5;
...
% fit the model
count = 1;
parfor count = 1:n_fit
    % for i = 1:n_fit
    rng(count, 'twister')
    flag = 0;
    while flag == 0
        try
            [Xfit{count}, E(count)] = fmincon(obFunc, rand(size(LB2)).*(UB2-LB2) + LB2, [], [], [], [], LB, UB, [], options);
            flag = 1;
        end
    end
