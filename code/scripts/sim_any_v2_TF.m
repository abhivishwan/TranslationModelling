function sub = sim_any_v2_TF(model_flag, X, expt)
...
switch model_flag
    ...
    case 2 % pure Kalman filter model
        ...
        [m,v] = compute_pureKalmanFilter_v1_TF( X, target(ind), feedback(ind), headAngle_feedback(ind) );
        sub.respond(ind) = target(ind) + m + sqrt(v) .* randn(sum(ind),1);

    case 3 % pure LAndmark navigation model
        %[m,v] = compute_KFModel_v1( X, target(ind), headAngle_t(ind), feedback(ind), headAngle_feedback(ind) );
        [m,v] = compute_pureLandmark_v1_TF( X, target(ind), feedback(ind), headAngle_feedback(ind) );
        sub.respond(ind) = target(ind) + m + sqrt(v) .* randn(sum(ind),1);
        
    case 4 % averaging model
        %[m,v] = compute_averagingModel_v1( X, target(ind), headAngle_t(ind), feedback(ind), headAngle_feedback(ind) );
        [m,v] = compute_averaging_v1_TF( X, target(ind), target(ind), feedback(ind), headAngle_feedback(ind) );
        sub.respond(ind) = target(ind) + m + sqrt(v) .* randn(sum(ind),1);
    ...
    case 5 % sampling model
        ...
        [m, v, p_true] = compute_sampling_v1_TF( X, target(ind), target(ind), feedback(ind), headAngle_feedback(ind) );
        r = rand(size(p_true));
        i2 = r < p_true;
        i1 = ~i2;
    ...
    case 6 % random sampling model
        %[m, v, p_true] = compute_samplingModel_v1( X, target(ind), headAngle_t(ind), feedback(ind), headAngle_feedback(ind) );
        [m, v, p_true] = compute_kalrand_v1_TF( X, target(ind), target(ind), feedback(ind), headAngle_feedback(ind) );
        r = rand(size(p_true));
        i2 = r < p_true;
        i1 = ~i2;
        sub.respond(ind) = target(ind) ...
