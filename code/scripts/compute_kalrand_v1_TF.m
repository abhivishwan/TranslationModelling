function [mean_error, var_error, p_true, logP, nofeedback_mean, nofeedback_var, feedback_mean, feedback_var, p_nofeedback, p_feedback] = compute_kalrand_v1_TF( X, target, headAngle_t, feedback, headAngle_feedback )
...
s_f         = X(7); % subject estimate of feedback noise
omega       = X(8); % subject pTrue

% compute p_true
% p_true = compute_pTrue_v1_TF( X, feedback, headAngle_feedback );
for ii = 1:length(actual_error)
    p_true(ii,1) = omega;
end
...
% nofeedback_var = target; % changing mean to the target
...
[feedback_mean, feedback_var] = compute_pureKalmanFilter_v1_TF( X(1:8), target, feedback, headAngle_feedback );
