...
true_heading  = [-6:1:1];
est_heading   = [-5 -4.2 -3.25 -1.85 est_target];
est_heading_s = [0.055 0.09  0.14  0.18  0.21];
...
ylim([0 5])
a = put_arrow(ax, [5 est_target], [2.5 2.5])
a = put_textbox(ax, [true_heading(end) true_target], [0.5 -0.5], 'measured error', fontsize, 'none', 'bottom', 'center')
a = put_doublearrow(ax, [true_heading(end) true_target], [0.5 0.5])
a = put_textbox(ax, [true_target 6], [3.5 2.5], 'respond when $m_t = X$', fontsize, 'none', 'bottom', 'center')
...
set(gca, 'ytick', 1:4, ...
    ...
    'tickdir', 'out')
grid on
xlabel('heading distance', 'interpreter', 'latex')
end

function pathIntegration_Old(ax)
...
true_heading  = [0:1:4];
est_heading   = [0 0.8 1.75 3.15 est_target];
est_heading_s = [0.055 0.09  0.14  0.18  0.21];
norm_factor = normpdf(0, 0, est_heading_s(1));
x_vals = [-1:0.01:6];
...
function landmarkNavigation_Old(ax)
...
feedback_s = 0.09;
ss = 1 / (1 / feedback_s^2 + 1 / est_heading_s^2);
mu = (feedback / feedback_s^2 + true_target / est_heading_s^2) * ss;

est_target = mu;

feedback_off = true_target/2;
est_heading2   = [nan nan feedback_off 3.15 mu ];
est_heading2_s = [nan nan 0.055  0.09  0.14 ];
norm_factor = normpdf(0, 0, est_heading2_s(3));
x_vals = [-1:0.01:6];

axes(ax); hold on; set(ax, 'ydir', 'reverse')
l = put_points( true_target, 1, 's', 10, AZred,  AZblue)
% l = put_points(  est_target, 2, 's', 10, AZblue, AZblue)
l = put_points(true_heading, 2, 'V', 10, gray,   AZred)

[l, l2] = put_pointsWithDist(    feedback, 3, feedback_s,     norm_factor, x_vals, '*', 10, AZsand, AZsand)
l = plot([0 feedback_off], [3 3], '-', 'color', AZsand,'LineWidth',8)
l = put_points(  est_target, 4, 's', 10, AZblue, AZblue)
[l, l2] = put_pointsWithDist(est_heading2, 5, est_heading2_s, norm_factor, x_vals, '^', 10, gray,   AZblue)

plot([1 1]*est_target, [4 5], '--', 'color', AZblue)
plot([1 1]*true_target, [0.5 1], '--', 'color', 'k')
plot([1 1]*true_heading(end), [0.5 2], '--', 'color', 'k')
plot([true_target true_target feedback feedback], [1 3.5 3.5 3], 'k-')
plot(est_target*[1 1], [3.5 4], 'k-')

ylim([0 6])
xlim([-1 6])

a = put_arrow(ax, [5 est_target], [4.5 4.5])
a = put_textbox(ax, [true_heading(end) true_target], [0.5 -0.5], 'measured error', fontsize, 'none', 'bottom', 'center')
a = put_doublearrow(ax, [true_heading(end) true_target], [0.5 0.5])
a = put_textbox(ax, [true_target 6], [5.5 4], 'respond when $m_t = X$', fontsize, 'none', 'middle', 'center')
a = put_arrow(ax, [1.5 est_heading2(end)], [3.75 3.75])
a = put_textbox(ax, [-0.6 1.4], [5.8 3.25], 'combine feedback with initial uncertainity of target', fontsize, 'none', 'middle', 'center')
