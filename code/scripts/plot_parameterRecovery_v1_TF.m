function [r_all,p_all] = plot_parameterRecovery_v1_TF( sim, model_flag, var_name, var_symbol, par_order )
...
for i = 1:length(on_ax);
    ...
    xlm = get(ax(i), 'xlim');
    l3(i) = plot(xlm, xlm, 'k--');
    [r_all(i),p_all(i)] = nancorr(X_sim(:,j), X_fit(:,j), 'spearman');
